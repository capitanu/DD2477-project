...
public class KGramIndex {
    ...
    public List<KGramPostingsEntry> intersect(List<KGramPostingsEntry> p1, List<KGramPostingsEntry> p2) {
		List<KGramPostingsEntry> rtn = new ArrayList<KGramPostingsEntry>();

		if ( p1 == null || p2 == null){
			return new ArrayList<KGramPostingsEntry>();
		}
		
		for(KGramPostingsEntry kgpe1 : p1){
			for(KGramPostingsEntry kgpe2 : p2){
				if(kgpe1.tokenID == kgpe2.tokenID)
					rtn.add(new KGramPostingsEntry(kgpe1));
			}
		}
        return rtn;
    }
    ...
    public void insert( String token ) {
		...
		for(int i = 0; i < token.length() - this.getK() + 1; i++){
			String gram = token.substring(i, i + this.getK());
			if(index.containsKey(gram)){
				boolean contains = false;
				for(KGramPostingsEntry el : index.get(gram)){
					if(el.tokenID == id)
						contains = true;
				}
				if(contains == false)
					index.get(gram).add(new KGramPostingsEntry(id));
			} else {
				index.put(gram, new ArrayList<KGramPostingsEntry>());
				index.get(gram).add(new KGramPostingsEntry(id));
			}
		}
    }
